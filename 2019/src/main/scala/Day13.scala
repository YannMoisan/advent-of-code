import Day13.ParameterMode.{Immediate, Position, Relative}
...
object Day13 extends SinglePuzzle[Int, Int] {
  override def part1(line: String): Int = {
    ...
    val outputs = computeOutput(Memory(program, mutable.Map.empty[Long, Long]), Array(1))
    outputs.grouped(3).count(seq => seq(2) == 2L)
  }
  ...
  private def computeOutput(program: Memory, inputs: Array[Long]): mutable.Seq[Long] = {
    var pointer      = 0L
    var inputPtr     = 0
    var continue     = true
