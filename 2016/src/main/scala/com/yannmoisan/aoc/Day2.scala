...
object Day2 extends Puzzle {
  ...
  def part1 = part(Pos(1, 1), keypad1)

  def part2 = part(Pos(2, 2), keypad2)

  def part(init: Pos, keypad: Seq[String]) = { lines: Seq[String] =>
    val parsed: Seq[Seq[Direction]] = lines.map(_.map(parse))
    val all = parsed.scanLeft(init) { case (acc, dirs) => move(acc, dirs, keypad) }
