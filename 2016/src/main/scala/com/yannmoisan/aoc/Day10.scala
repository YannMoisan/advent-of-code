...
object Day10 extends Puzzle {
  ...
  case class Bot(id: String, low: Option[Int], high: Option[Int]) {
    def addChip(chipId: Int): Bot = this match {
      ...
      case Bot(_, Some(curChipId), None) => this.copy(
        low = Some(min(chipId, curChipId)),
        high = Some(max(chipId, curChipId)))
    }
  ...
  case class State(bots: Map[String, Bot], outputs: Map[String, Output], is: Seq[Instruction], cond: Option[String]) {
  ...
  object Recipient {
    def apply(typ: String, id: String): Recipient = if (typ == "bot") BotRecipient(id) else OutputRecipient(id)
  }
  ...
  case class GiveFromBot(botId: String, lowRecipient: Recipient, highRecipient: Recipient) extends Instruction
    ...
    val updates : Seq[State => State] = next match {
      ...
      case GiveFromBot(botId, lowRecipient, highRecipient) =>
        Seq(
          ...
          s => s.copy(cond = if (s.bots(botId).low.get == 17 && s.bots(botId).high.get == 61) Some(botId) else None),
