object Day24 extends MultiPuzzle[Int, Int] {
  ...
  def moves: State => Seq[State] =
    s => possibleMoves(s.p).filter(p => validPos(p, s.grid)).map(p => State(s.grid, p))

  case class State(grid: Grid, p: Pos)

  // no need to convert char to int
  def findNumbers(grid: Grid): Seq[(Int, Pos)] =
    for {
  ...
  override def part1(lines: Iterator[String]) = part(false)(lines.toList)

  override def part2(lines: Iterator[String]) = part(true)(lines.toList)
}
