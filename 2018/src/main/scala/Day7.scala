object Day7 extends MultiPuzzle[String, String] {
  ...
  override def part2(iter: Iterator[String]): String = {
    ...
    while (!instructions.isEmpty || !workers.isEmpty) {
      ...
      val canBegin = allSteps
        .filterNot(step => instructions.map(_._2).contains(step)).sorted.filterNot(e =>
          workers.map(_._1).contains(e)
        ) //.head
      ...
      workers = workers ++ toAdd.map(s => (s, time + 60 + s.head.toInt - 'A'.toInt + 1))
