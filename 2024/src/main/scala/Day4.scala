...
object Day4 extends MultiPuzzle[Int, Int] {
  override def part1(input: Iterator[String]): Int = {
    ...
    } yield (0 to 3).map(i => Pos(x + i, y)).map(grid(_)).mkString

    val verticalWords = for {
      x <- 0 until grid.dim.width
      y <- 0 until grid.dim.height - 3
    } yield (0 to 3).map(i => Pos(x, y + i)).map(grid(_)).mkString

    val fstDiagonalWords = for {
      x <- 0 until grid.dim.width - 3
      y <- 0 until grid.dim.height - 3
    } yield (0 to 3).map(i => Pos(x + i, y + i)).map(grid(_)).mkString

    val sndDiagonalWords = for {
      x <- 0 until grid.dim.width - 3
      y <- 0 until grid.dim.height - 3
    } yield (0 to 3).map(i => Pos(x + 3 - i, y + i)).map(grid(_)).mkString
